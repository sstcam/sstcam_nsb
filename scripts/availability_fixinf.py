...
def calc_field_rotation(Lat,Azs,Alts):
    ...
    Azs=Azs*np.pi/180.0
    nonvalid=np.where(np.logical_or(abs(Alts-90.0)<0.2,abs(Alts-270.0)<0.2))
    print(Alts,nonvalid,np.amax(Alts),np.amin(Alts))

    Alts=Alts*np.pi/180.0
    Lat=Lat*np.pi/180.0 #Must be in rad for numpy
    rots=K*np.cos(Lat)*np.cos(Azs)/np.cos(Alts)
    rots[nonvalid]=np.inf
    return rots
...
if __name__=='__main__':
    ...
    alts=sc.alt.deg
    azs=sc.az.deg
    Altvals=np.linspace(20,89.2,1000)
    Azvals=np.linspace(0,360,1000)
    #Azvals=np.concatenate((np.linspace(0,70,250),np.linspace(110.0,250.0,500),np.linspace(290,360,250)))
    Azs,Alts=np.meshgrid(Altvals,Azvals)
    print(Azs,Alts)
